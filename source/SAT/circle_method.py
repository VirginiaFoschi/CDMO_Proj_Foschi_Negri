...
def circle_method(n_teams: int) -> Tuple[List[List[int]], List[List[int]], List[List[int]]]:
    ...
    Returns
    -------
    base_a          : base_a[w][k]  -> 0-indexed team on the 'a' side of match k in week w
    base_b          : base_b[w][k]  -> 0-indexed team on the 'b' side
    team_match_idx  : team_match_idx[t][w] -> match index k that team t plays in week w
    """
    n_weeks   = n_teams - 1
    n_periods = n_teams // 2
    ...
    base_a = []
    base_b = []
    team_match_idx = [[-1] * n_weeks for _ in range(n_teams)]
